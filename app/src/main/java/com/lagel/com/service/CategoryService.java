...
public class CategoryService extends Service {
	...
	public void onCreate() {
		super.onCreate();
		String userAgent = String.format(USER_AGENT, getAppVersion(CategoryService.this));
		this.restClient = new RestServiceClient(userAgent);
		INTERVAL= 1000*60*60*24;  //48 hour
		initJobs();
		jobSync.start();
	}
	...
	private PendingIntent getActionIntent(String action) {
		Intent intent = new ActionIntent(action, this, CategoryService.class);
	...
	private class JobSync extends Job {
		...
		public void run() {
			try {
				getLock().acquire();
				if (Common.haveInternet(CategoryService.this))
				{
						...
						switch (categoryMainPojo.getCode()) {
							...
							case "200":
								if (categoryMainPojo.getData() != null && categoryMainPojo.getData().size() > 0) {
									...
									CategoryData.instance(CategoryService.this).clearDataCate();
									CategoryData.instance(CategoryService.this).setInsertCate(categoryMainPojo.getData());
								}
								break;
